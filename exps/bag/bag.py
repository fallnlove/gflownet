...
class BagMDP(bagmdp.BagMDP):
    ...
    def generate_modes(self, mode_fn):
        ...
        for char in self.alphabet:
            ...
            fillers = [
                "".join(s) for s in itertools.product(other_chars, repeat=other_size)
            ]
            random.shuffle(fillers)

            modes += [
                self.substruct_size * char + extra
                for extra in fillers[: int(self.top_frac * len(fillers))]
            ]
