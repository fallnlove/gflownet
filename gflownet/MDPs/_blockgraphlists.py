...
class GraphLists:
    ...
    def add_edge(self, n1, n2, features):
        ...
        assert n1 < len(self) and n2 < len(self), f"{n1=}, {n2=}, {len(self)=}"
...
class BGLeditor:
    def __init__(self, blocks_file):
        ...
        self.smi_to_natm = {
            smi: natm for smi, natm in zip(self.block_smi, self.block_natm)
        }
    ...
    def delete_blocks(self, bgl, block_mask):
        ...
        for bond in c.jbonds:
            if block_mask[bond[0]] and block_mask[bond[1]]:
                jbonds.append(
                    np.array([reindex[bond[0]], reindex[bond[1]], bond[2], bond[3]])
                )
    ...
    def atom_num_to_id_map(self):
        atom_nums = set(
            atom.GetAtomicNum() for mol in self.block_mols for atom in mol.GetAtoms()
        )
...
def randomwalk(length=8):
    ...
    while bgl.numblocks < length:
        ...
        bgl = editor.add_block(
            bgl, block_id=block_id, stem_idx=stem_idx, new_atom_idx=0
        )
