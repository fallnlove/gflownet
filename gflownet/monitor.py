...
class Monitor:
  ...
  """
    ...
    def eval_recent_rounds(self, model, round_num, allXtoR, log_slow):
        ...
        recent_samples = chain(
            self.sample_log[rd] for rd in recent_rounds if rd in self.sample_log
        )
        tolog = self.__evaluate_samples(
            recent_samples, round_num, model, allXtoR, name="recent", log_slow=log_slow
        )
        return tolog
    ...
    def __evaluate_samples(
        self, batch, round_num, model, allXtoR, name, log_slow=False
    ):
        ...
        if self.is_mode_f is not None:
            unique_modes = set(x for x, r in zip(xs, rewards) if self.is_mode_f(x, r))
            tolog.update(
                {
        ...
        if name == "recent" and log_slow:
            tolog.update(
                {
                    f"{name} - diversity": mean_pair_diversity(xs, self.dist_func),
                }
            )
        ...
        if len(rewards) > 0:
            ...
            ad_score = anderson_darling(rewards, self.target.ad_samples)
  ...
  """
    ...
    def eval_topk(self, k=100, log_slow=False):
        ...
        tolog = {
            "TopK performance": np.mean(rs),
        }
        if log_slow:
            tolog.update(
                {
                    "TopK diversity": mean_pair_diversity(xs, self.dist_func),
                }
            )
        return tolog
