...
def guide_sample(tools, x, allXtoR):
    ...
    """
        filtered_X = [item for item in filtered_X if tools.mdp_f.is_member(node, item)]
        traj.append(node)
...
def unique_children_in_x_guide_score(state, x, X, allXtoR, get_unique_children):
...
def probs_substructure(tools, node, x, X, allXtoR):
    ...
    """
    valid_children, scores = tools.mdp_f.unique_children_in_x_guide_score(
        node, x, X, allXtoR, tools.mdp_f.get_unique_children
    )
    if sum(scores) > 0:
...
def probs_uniform(tools, node, x, X, allXtoR):
    valid_children = tools.mdp_f.get_unique_children_in_x(
        node, x, tools.mdp_f.get_unique_children
    )
...
class RayManager:
    ...
    def __init__(self, args, mdp):
        ...
        if args.model == "sub" and args.guide != "substructure":
            print(
                f"WARNING: Substructure GFN model chosen, but guide is not substructure"
            )
  ...
  """
    ...
    def __update(self):
        ...
        """
        done, notdone = ray.wait(self.futures, num_returns=len(self.futures), timeout=0)
        ...
        garbage_collect()
        print(
            f"RayManager - in jobs {self.job_stack.qsize()}; results {self.results_qu.qsize()}; active tasks {len(self.futures)}"
        )
        return
